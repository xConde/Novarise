 ...
 * Follows StS design: each level adds ONE specific twist.
 */
...
export const ASCENSION_LEVELS: ReadonlyArray<AscensionLevel> = [
  ...
  { level: 11, label: 'Scarce', description: 'One fewer relic choice offered', effect: { type: AscensionEffectType.FEWER_RELIC_CHOICES, value: 1 } },
...
export function getAscensionEffects(level: number): Map<AscensionEffectType, number> {
  ...
  for (let i = 0; i < clampedLevel; i++) {
    ...
    const current = effects.get(asc.effect.type);

    switch (asc.effect.type) {
      ...
      case AscensionEffectType.SHOP_PRICE_MULTIPLIER:
        effects.set(asc.effect.type, (current ?? 1) * asc.effect.value);
        break;
      ...
      case AscensionEffectType.FEWER_RELIC_CHOICES:
        effects.set(asc.effect.type, (current ?? 0) + asc.effect.value);
        break;
    }
  }
