...
describe('EncounterService', () => {
  ...
  it('prepareEncounter() for COMBAT node returns correct EncounterConfig shape', () => {
    // Waves are always procedurally generated for all map ids
  ...
  it('prepareEncounter() for COMBAT node always delegates to generateCombatWaves', () => {
    // Procedural generation applies regardless of campaignMapId
    const node = makeMapNode({ type: NodeType.COMBAT, campaignMapId: 'campaign_05' });
  ...
  // ── generateWavesForNode: procedural routing ─────────────────────────────

  it('UNKNOWN node type uses the same combat path and calls generateCombatWaves', () => {
    const node = makeMapNode({ type: NodeType.UNKNOWN, campaignMapId: 'campaign_01' });
