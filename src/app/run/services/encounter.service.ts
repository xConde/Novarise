 ...
 *
 * This service is root-scoped to survive the /run route transition.
 */
...
export class EncounterService {
  ...
  constructor(
    ...
    private readonly runMapService: RunMapService,
  ) {}
  ...
  loadEncounterMap(config: EncounterConfig): void {
    const mapState = this.runMapService.loadLevel(config.campaignMapId);
    if (mapState === null) {
      ...
      throw new Error(
        ...
        `Check CAMPAIGN_MAP_TIERS in run.constants.ts.`,
      );
    }
