...
import { createSeededRng } from '../constants/ascent.constants';
...
export class DeckService {
  ...
  resetForEncounter(): void {
    ...
    // Exhausted cards stay exhausted for the encounter
    this.deckState = {
  ...
  /** Draw cards for a new wave. Refills energy and draws up to handSize. */
  drawForWave(): void {
    this.energyState = { current: this.energyState.max, max: this.energyState.max };
    const toDraw = DECK_CONFIG.handSize;
  ...
  /** Discard entire hand at end of wave. */
  discardHand(): void {
    this.deckState = {
      ...this.deckState,
      discardPile: [...this.deckState.discardPile, ...this.deckState.hand],
      hand: [],
    };
    this.emit();
  }
  ...
  /**
   * Play a card from hand. Deducts energy and moves card to discard.
