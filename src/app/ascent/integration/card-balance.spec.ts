...
describe('Card System — Balance', () => {
  ...
  it('starter deck should have exactly 10 cards', () => {
    expect(STARTER_IDS.length).toBe(10);
  });
  ...
  it('starter deck energy cost total should be 14-18 (playable across ~5-6 energy waves)', () => {
    // 4×BASIC(1) + 2×SNIPER(3) + 1×SPLASH(2) + 1×SLOW(2) + 1×GOLD_RUSH(1) + 1×DAMAGE_BOOST(1) = 16
    const totalCost = STARTER_DEFS.reduce((sum, d) => sum + d.energyCost, 0);
    expect(totalCost).toBeGreaterThanOrEqual(14);
    expect(totalCost).toBeLessThanOrEqual(18);
  });
  ...
  it('should have exactly 25 card definitions', () => {
  ...
  it('starter cards should all have STARTER rarity', () => {
    ...
    // GOLD_RUSH and DAMAGE_BOOST are in starter deck but have COMMON/COMMON rarity
    // That is intentional — they can also appear as rewards. Count must be <= 2
    expect(nonStarterRarity.length).toBeLessThanOrEqual(2);
  });
